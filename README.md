...
- **헤더(모바일)**: 영문 모토는 **`1rem`**, 메뉴 링크는 **`0.9rem`**으로 맞춰 가독성을 올렸습니다. 아주 좁은 폭(**400px / 340px** 이하)에서는 모토만 **`0.93rem` → `0.88rem`** 순으로 살짝 줄여 **한 줄**에 가깝게 유지합니다. 모토 왼쪽의 **`star_light.png`**는 `We` 앞·위에서 비추는 위치로 잡았습니다.
...
넷플릭스 느낌의 단일 페이지, 시맨틱 태그, 카드 5개, 모바일 미디어 쿼리, 히어로·모토·별 이미지, 추천/최신 두 줄, `id`·내비·푸터 문구 정리, 모바일 모토 글자 크기·한 줄 맞춤 조정, README·커밋 요청 등 — 대화 과정에서 단계적으로 반영했습니다.
